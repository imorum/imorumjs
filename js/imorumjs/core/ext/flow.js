infrajs.merge = function _merge(target) {
    target = target || {};
...
infrajs.forIn = function _forIn(obj, callback) {
    for (var x in obj) {
        //if (obj.hasOwnProperty(x)) callback(obj[x], x);
        callback(obj[x], x);
    }
}
infrajs.foreach = function _foreach(arr, callback, start) {
    var cancelled;
    if (arr) {
        // javascript array
...
infrajs.callIf = function _callIf(obj, name, args) {
